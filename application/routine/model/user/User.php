...
class User extends ModelBasic
{
    ...
    public static function backOrderBrokerage($orderInfo)
    {
        ...
        //file_put_contents(LOG_PATH.DS.'yongjin.log','开始测试----'.PHP_EOL,FILE_APPEND);
        ...
        //file_put_contents(LOG_PATH.DS.'yongjin.log','分销状态：'.$storeBrokerageStatu.''.PHP_EOL,FILE_APPEND);

        $brokerageRatio = (SystemConfigService::get('store_brokerage_ratio') ?: 0)/100;
        //file_put_contents(LOG_PATH.DS.'yongjin.log','佣金比例：'.$brokerageRatio.''.PHP_EOL,FILE_APPEND);
        if($brokerageRatio <= 0) return true;
        $cost = isset($orderInfo['cost']) ? $orderInfo['cost'] : 0;//成本价
        //file_put_contents(LOG_PATH.DS.'yongjin.log','成本价：'.$cost.''.PHP_EOL,FILE_APPEND);
        ...
        //file_put_contents(LOG_PATH.DS.'yongjin.log','佣金：'.$brokeragePrice.''.PHP_EOL,FILE_APPEND);
